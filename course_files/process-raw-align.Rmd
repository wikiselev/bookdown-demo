...
```{r Process-raw-align0, include=FALSE, cache=TRUE, cache.extra = list(R.version, sessionInfo())}
library(knitr)
...
![Figure 1: Diagram of how STAR performs alignments, taken from Dobin et al.](figures/STAR_explanation.png)
...
```{bash Process-raw-align1, eval=FALSE}
mkdir indices
mkdir indices/STAR
...
```{bash Process-raw-align2, eval=FALSE}
mkdir results
mkdir results/STAR
...
![Figure 2: The 7-mers derived from an example read](figures/Kmers.png)
...
![Figure 3: A diagram explaining Kallisto's Equivalence Classes, taken from https://pachterlab.github.io/kallisto/singlecell.html.](figures/TCC.jpg)
...
```{bash Process-raw-align3, eval=FALSE}
...
```{bash Process-raw-align4, eval=FALSE}
