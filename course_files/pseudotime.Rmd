...
```{r pseudotime2, out.width = '90%', fig.cap="Descriptions of trajectory inference methods for single-cell transcriptomics data (Fig. 2 from Cannoodt et al, 2016).", echo=FALSE}
knitr::include_graphics("figures/cannoodt_pseudotime_properties.png")
```

```{r pseudotime3, out.width = '90%', fig.cap="Characterization of trajectory inference methods for single-cell transcriptomics data (Fig. 3 from Cannoodt et al, 2016).", echo=FALSE}
...
```{r pseudotime8, m3d-select-genes}
...
dCellData <- new_cell_data_set(d, cell_metadata = pd, gene_metadata =  fd)
dCellData <- preprocess_cds(dCellData, num_dim = 50)
dCellData <- estimate_size_factors(dCellData)
dCellDataSet <- reduce_dimension(dCellData, reduction_method = "UMAP")
dCellDataSet <- cluster_cells(dCellDataSet, reduction_method = "UMAP")
dCellDataSet <- learn_graph(dCellDataSet)
root_group <- colnames(dCellDataSet)[clusters(dCellDataSet) == 1]
dCellDataSet <- order_cells(dCellDataSet, root_cells = root_group)
plot_cells(dCellDataSet)
```
```{r pseudotime10}
pseudotime_monocle <-
    data.frame(
        Timepoint = dCellDataSet@colData$timepoint,
        pseudotime = dCellDataSet@principal_graph_aux$UMAP$pseudotime
...
```{r pseudotime13, message=FALSE, warning=FALSE}
library("lle")
...
```{r pseudotime19, warning=FALSE, message=FALSE, result='hide'}
...
__TSCAN__
```{r pseudotime28, message=FALSE, warning=FALSE}
