...
```{r exprs-qc8, fig.cap = "Histogram of library sizes for all cells"}
hist(
    umi$total_counts,
    breaks = 100
)
...
```{r exprs-qc10, fig.cap = "Histogram of the number of detected genes in all cells"}
hist(
...
```{r exprs-qc12, fig.cap = "Percentage of counts in MT genes"}
plotColData(
    umi,
...
```{r exprs-qc13, fig.cap = "Percentage of counts in ERCCs"}
plotColData(
    umi,
    x = "total_features_by_counts",
    y = "pct_counts_ERCC",
...
```{r exprs-qc17, fig.align='center', fig.cap="PCA plot used for automatic detection of cell outliers", message=FALSE, warning=FALSE, out.width='90%'}
umi <- runPCA(
    umi, 
    use_coldata = TRUE, 
...
```{r exprs-qc20, fig.cap = "Comparison of the default, automatic and manual cell filters", echo=FALSE}
library(limma)
...
```{r exprs-qc21, fig.cap = "Number of total counts consumed by the top 50 expressed genes", fig.asp = 1}
