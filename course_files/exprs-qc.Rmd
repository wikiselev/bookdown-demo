...
```{r exprs-qc10}
umi_cell <- perCellQCMetrics(umi)

umi_feature <- perFeatureQCMetrics(umi)

umi <- addPerCellQC(umi)

umi <- addPerFeatureQC(umi)

#umi_feature <- perFeatureQCMetrics(
#  umi, 
#  subsets = list(
#    ERCC = altExp(umi, "spike-in") <- SummarizedExperiment(SpikeIn),
#    MT = altExp(umi, "MTgenes") <- SummarizedExperiment(MTgenes)
#  )
#)
...
filter_by_total_counts <- (umi_cell$total > 25000)
...
filter_by_expr_features <- (umi_feature$detected > 7000)
