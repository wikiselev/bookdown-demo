...
```{r confounders-reads0, echo=FALSE}
library(knitr)
opts_chunk$set(cache = TRUE, out.width='90%', fig.align = 'center')
```

```{r confounders-reads1, message=FALSE, warning=FALSE}
...
```{r confounders-reads2, fig.cap = "PCA plot of the tung data"}
reads.qc <- runPCA(reads.qc, exprs_values = "logcounts_raw")
dim(reducedDim(reads.qc, "PCA"))
plotPCA(reads.qc, colour_by = "replicate", size_by = "sum", shape_by = "individual")
```
...
```{r confounders-reads3, fig.cap = "PC correlation with the number of detected genes", fig.asp=1}
logcounts(reads.qc) <- assay(reads.qc, "logcounts_raw")
plotExplanatoryPCs(reads.qc,variables = "detected")
...
```{r confounders-reads4, fig.cap = "Explanatory variables"}
plotExplanatoryVariables(reads.qc,exprs_values = "logcounts_raw",
...
```{r confounders-reads5, echo=FALSE}
sessionInfo()
```
