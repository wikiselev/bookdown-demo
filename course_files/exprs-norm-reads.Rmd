...
```{r exprs-norm-reads0, echo=FALSE}
...
```{r exprs-norm-reads1, eval=FALSE}
...
```{r exprs-norm-reads2, eval=FALSE}
...
```{r exprs-norm-reads3, eval=FALSE}
...
```{r exprs-norm-reads4, eval=FALSE}
...
```{r exprs-norm-reads5, eval=FALSE}
...
```{r exprs-norm-reads6, message=FALSE, warning=FALSE}
library(scRNA.seq.funcs)
library(scater)
library(scran)
...
### Raw
```{r exprs-norm-reads7, fig.cap = "PCA plot of the tung data"}
reads.qc <- runPCA(reads.qc,exprs_values = "logcounts_raw")
plotPCA(reads.qc,colour_by = "batch", size_by = "detected", shape_by = "individual")
```

### CPM
```{r exprs-norm-reads8, fig.cap = "PCA plot of the tung data after CPM normalisation"}
logcounts(reads.qc) <- log2(calculateCPM(reads.qc) + 1)
plotPCA(reads.qc, colour_by = "batch", size_by = "detected", shape_by = "individual")
```

```{r exprs-norm-reads9, fig.cap = "Cell-wise RLE of the tung data"}
plotRLE(reads.qc, exprs_values = "logcounts_raw",colour_by = "batch")
plotRLE(reads.qc, exprs_values = "logcounts",colour_by = "batch")
```

### scran
```{r exprs-norm-reads10, fig.cap = "PCA plot of the tung data after LSF normalisation"}
qclust <- quickCluster(reads.qc, min.size = 30)
reads.qc <- computeSumFactors(reads.qc, sizes = 15, clusters = qclust)
reads.qc <- logNormCounts(reads.qc) ## normalize is defunct
plotPCA(reads.qc, colour_by = "batch",size_by = "detected", shape_by = "individual")
```

```{r exprs-norm-reads11, fig.cap = "Cell-wise RLE of the tung data"}
plotRLE(reads.qc, exprs_values = "logcounts_raw",colour_by = "batch")
plotRLE(reads.qc, exprs_values = "logcounts",colour_by = "batch")
```
...
```{r exprs-norm-reads12}
summary(sizeFactors(reads.qc))
```
...
```{r exprs-norm-reads13, fig.cap = "PCA plot of the tung data after downsampling"}
logcounts(reads.qc) <- log2(Down_Sample_Matrix(counts(reads.qc)) + 1)
plotPCA(reads.qc,colour_by = "batch",size_by = "detected", shape_by = "individual")
```

```{r exprs-norm-reads14, fig.cap = "Cell-wise RLE of the tung data"}
plotRLE(reads.qc, exprs_values = "logcounts_raw",colour_by = "batch")
plotRLE(reads.qc, exprs_values = "logcounts",colour_by = "batch")
```
...
```{r exprs-norm-reads15, message = FALSE, warning = FALSE}
...
```{r exprs-norm-reads17}
eff_length <- abs(rowData(reads.qc.ann)$end_position - rowData(reads.qc.ann)$start_position) / 1000
```

```{r exprs-norm-reads18, fig.cap = "Gene length vs Mean Expression for the raw data"}
plot(eff_length, rowMeans(counts(reads.qc.ann)))
```
...
```{r exprs-norm-reads19}
tpm(reads.qc.ann) <- log2(calculateTPM(reads.qc.ann, eff_length) + 1)
```

Plot the results as a PCA plot:
```{r exprs-norm-reads20, fig.cap = "PCA plot of the tung data after TPM normalisation"}
tmp <- runPCA(reads.qc.ann, exprs_values = "tpm")
plotPCA(tmp,colour_by = "batch",size_by = "detected",shape_by = "individual")
```

```{r exprs-norm-reads21}
tpm(reads.qc.ann) <- log2(calculateFPKM(reads.qc.ann, eff_length) + 1)
```

```{r exprs-norm-reads22, fig.cap = "PCA plot of the tung data after FPKM normalisation"}
tmp <- runPCA(reads.qc.ann, exprs_values = "tpm")
plotPCA(tmp,colour_by = "batch",size_by = "detected",shape_by = "individual")
```
...
```{r exprs-norm-reads23, echo=FALSE}
sessionInfo()
```
