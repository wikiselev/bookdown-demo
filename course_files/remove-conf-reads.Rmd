...
```{r remove-conf-reads0, echo=FALSE}
library(knitr)
opts_chunk$set(cache= TRUE, fig.align = 'center')
```

```{r remove-conf-reads1, message=FALSE, warning=FALSE}
...
```{r remove-conf-reads3, message=FALSE, warning=FALSE}
ruv_counts <- rbind(counts(reads.qc),counts(altExp(reads.qc))) ## RUV assumes raw counts with ERCCs in it 

ruvg1 <- RUVg(ruv_counts, erccs, k = 1)
ruvg1_nc <- ruvg1$normalizedCounts[grep("^ENS", rownames(ruvg1$normalizedCounts)),]
assay(reads.qc, "ruvg1") <- log2(t(t(ruvg1_nc) / colSums(ruvg1_nc) * 1e6) + 1)

ruvg10 <- RUVg(ruv_counts, erccs, k = 10)
ruvg10_nc <- ruvg10$normalizedCounts[grep("^ENS", rownames(ruvg10$normalizedCounts)),]
assay(reads.qc, "ruvg10") <- log2(t(t(ruvg10_nc) / colSums(ruvg10_nc) * 1e6) + 1)
```

#### RUVs

```{r remove-conf-reads4}
...
assay(reads.qc, "ruvs1") <- log2(t(t(ruvs1_nc) / colSums(ruvs1_nc) * 1e6) + 1)
...
assay(reads.qc, "ruvs10") <- log2(t(t(ruvs10_nc) / colSums(ruvs10_nc) * 1e6) + 1)
```
...
```{r remove-conf-reads5, message=FALSE, warning=FALSE, paged.print=FALSE}
...
mod2 = model.matrix(~ reads.qc$detected, data = mod_data)
...
```{r remove-conf-reads6, eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r remove-conf-reads7}
mnn_out <- fastMNN(reads.qc,batch = reads.qc$individual)
assay(reads.qc, "mnn") <- assay(mnn_out,'reconstructed')
```
...
```{r remove-conf-reads9, echo=FALSE}
...
```{r remove-conf-reads10}
reads.qc = runPCA(reads.qc, exprs_values = 'logcounts', ncomponents = 20)
pca <- as.matrix(reducedDim(reads.qc,'PCA'))
harmony_emb <- HarmonyMatrix(pca, reads.qc$batch, theta=2, do_pca=FALSE)
reducedDim(reads.qc,'harmony') <- harmony_emb

plotReducedDim(reads.qc, dimred = 'harmony', colour_by = "batch", size_by = "detected", shape_by = "individual")
```
...
```{r remove-conf-reads11}
for(n in assayNames(reads.qc)) {
    tmp <- runPCA(reads.qc, exprs_values = n, ncomponents = 20)
...
```{r remove-conf-reads13, message = FALSE, warning = FALSE}
...
```{r remove-conf-reads14, message = FALSE}
library("reshape2")
...
```{r remove-conf-reads15, echo=FALSE}
sessionInfo()
```
