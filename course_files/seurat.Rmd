...
```{r}
# please run this in your R session interactively
# it does not work as a script
...
```{r}
FeaturePlot(object = pbmc, features = c("MS4A1", "GNLY", "CD3E", "CD14", "FCER1A", "FCGR3A", "LYZ", "PPBP", "CD8A"), cols = c("grey", "blue"))
```
...
UMAPPlot(object = pbmc, do.label = TRUE, pt.size = 0.5)
```
...
```{r}
# Demonstration of how to plot two UMAP plots side by side, and how to color
# points based on different criteria
plot1 <- UMAPPlot(object = pbmc, do.return = TRUE, no.legend = TRUE, do.label = TRUE)
plot2 <- UMAPPlot(object = pbmc, do.return = TRUE, group.by = "ClusterNames_0.6", no.legend = TRUE, do.label = TRUE)
