...
```{r imputation2, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r imputation7, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r imputation8}
deng <- logNormCounts(deng)
res <- DrImpute(deng@assays@data$logcounts, ks=8:12)
...
```{r imputation9, eval=FALSE}
### NEED TO CREATE ENV CALLED magic_env
### NEED TO pip install python-magic and magic-impute
library(reticulate)
use_condaenv('magic_env')
py_discover_config(required_module = "magic")
magic <- reticulate::import('magic')
res <- magic(t(deng@assays@data$logcounts), genes="all_genes", knn=10, t="auto")
```

```{r imputation10, eval=FALSE}
res <- t(as.matrix(res))
rownames(res) <- rownames(deng)
colnames(res) <- colnames(deng)
res <- SingleCellExperiment(
...
To evaluate the impact of the imputation, we use `SC3` to cluster the imputed matrix
```{r imputation11, message=FALSE, warning=FALSE, paged.print=FALSE, eval=FALSE}
res <- sc3_estimate_k(res)
metadata(res)$sc3$k_estimation
res <- sc3(res, ks = 10, n_cores = 1, gene_filter = FALSE)
adjustedRandIndex(colData(deng)$cell_type2, colData(res)$sc3_10_clusters)
