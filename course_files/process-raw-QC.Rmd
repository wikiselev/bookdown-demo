...
```{r process-raw-QC0, include=FALSE, cache=TRUE, cache.extra = list(R.version, sessionInfo())}
library(knitr)
...
```{bash process-raw-QC1, eval=FALSE}
...
```{bash process-raw-QC2, eval=FALSE, collapse=TRUE}
fastqc -h
```
...
```{bash process-raw-QC3, eval=FALSE, echo = TRUE}
...
```{bash process-raw-QC4, eval=FALSE}
trim_galore -h
```
...
```{bash process-raw-QC5, eval=FALSE}
