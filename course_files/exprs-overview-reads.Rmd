...
```{r exprs-over-reads0, echo=FALSE}
library(knitr)
opts_chunk$set(cache = TRUE, out.width='90%', fig.align = 'center')
```

```{r exprs-over-reads1, message=FALSE, warning=FALSE}
library(SingleCellExperiment)
library(scater)
options(stringsAsFactors = FALSE)
reads <- readRDS("data/tung/umi.rds")
reads.qc <- reads[rowData(reads)$use,! colData(reads)$discard]
```
...
```{r exprs-over-reads2, echo=FALSE, fig.cap="Schematic representation of PCA dimensionality reduction", out.width='100%'}
...
```{r exprs-over-reads3, fig.cap = "PCA plot of the tung data"}
reads <- runPCA(reads, exprs_values = "counts")
dim(reducedDim(reads, "PCA"))
plotPCA(reads, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```

With log-transformation:
```{r exprs-over-reads4, fig.cap = "PCA plot of the tung data"}
reads <- runPCA(reads, exprs_values = "logcounts_raw")
dim(reducedDim(reads, "PCA"))
plotPCA(reads, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```
...
```{r exprs-over-reads5, fig.cap = "PCA plot of the tung data"}
reads.qc <- runPCA(reads.qc, exprs_values = "logcounts_raw")
dim(reducedDim(reads.qc, "PCA"))
plotPCA(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```
...
```{r exprs-over-reads6, fig.cap = "PCA plot of the tung data (14214 genes)", echo=FALSE}
reads.qc <- runPCA(reads.qc, exprs_values = "logcounts_raw",ntop = nrow(reads.qc))
dim(reducedDim(reads.qc, "PCA"))
plotPCA(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```

```{r exprs-over-reads7, expr-overview-pca-after-qc-exercise1-2, fig.cap = "PCA plot of the tung data (50 genes)", echo=FALSE}
reads.qc <- runPCA(reads.qc, exprs_values = "logcounts_raw",ntop = 50)
dim(reducedDim(reads.qc, "PCA"))
plotPCA(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```
...
```{r exprs-over-reads8, fig.cap = "tSNE map of the tung data"}
set.seed(123456)
reads <- runTSNE(reads, exprs_values = "logcounts_raw", perplexity = 130)
plotTSNE(reads, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```

#### After QC

```{r exprs-over-reads9, fig.cap = "tSNE map of the tung data"}
set.seed(123456)
reads.qc <- runTSNE(reads.qc, exprs_values = "logcounts_raw", perplexity = 130)
plotTSNE(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```
...
```{r exprs-over-reads10, fig.cap = "tSNE map of the tung data (perplexity = 10)", echo=FALSE}
set.seed(123456)
reads.qc <- runTSNE(reads.qc, exprs_values = "logcounts_raw", perplexity = 10)
plotTSNE(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```

```{r exprs-over-reads11, fig.cap = "tSNE map of the tung data (perplexity = 200)", echo=FALSE}
set.seed(123456)
reads.qc <- runTSNE(reads.qc, exprs_values = "logcounts_raw", perplexity = 200)
plotTSNE(reads.qc, colour_by = "replicate", size_by = "detected", shape_by = "individual")
```
...
```{r exprs-over-reads12, echo=FALSE}
sessionInfo()
```